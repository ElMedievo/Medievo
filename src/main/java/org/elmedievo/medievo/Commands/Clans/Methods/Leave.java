...
public class Leave {
    public static void removePlayerFromClan(Player player) {
        ...
        if (!Objects.requireNonNull(player_clan).equals("none")) {
            if (!Objects.requireNonNull(ClanLeaderGetter.getClanLeaderUUID(player_clan)).equals(player.getUniqueId().toString())) {
                String leader_name = ClanLeaderGetter.getClanLeaderName(player_clan);
                PlayerClanSetter.setPlayerClan(player.getUniqueId(), "none");
