...
public class Invite {
    public static void createPlayerToPlayerInvite(String inviter_name, String invited_name) {
        ...
        String inviters_clan = PlayerClanGetter.getPlayerClan(inviter_player.getUniqueId());
        if (playerIsOnline(invited_name)) {
            Player invited_player = Bukkit.getPlayer(invited_name);
            String inviteds_clan = PlayerClanGetter.getPlayerClan(invited_player.getUniqueId());
            if (Objects.requireNonNull(inviteds_clan).equals("none")) {
                if (!ClanQueue.isAlreadyBeingInvited(invited_name)) {
                    CreateQueues.inviteQueue.put(invited_name, inviter_name);
